...
export async function login(user_id: string, password: string) {
	...
	if (response.ok) {
		const { sub: id, name, email, role, proj: projects } = await response.json();
...
export function parse_authn_token(token: string): LexAuthUser {
	let { proj, sub, ...user } = jwtDecode<any>(token);
	user.projects = proj;
	user.id = sub;
	return user;
}
