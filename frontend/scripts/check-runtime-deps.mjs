...
// external, so a runtime import of anything else would fail at request time. Run after `vite build`.
...
if (unused.length) console.warn(`never imported at runtime, could be devDependencies: ${unused.join(', ')}`);
if (leaked.length) process.exit(1);
console.log(`${files.length} server files import only "dependencies"`);
