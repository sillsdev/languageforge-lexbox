...
// -- Field resolution (how API view data is merged with built-in defaults) --
...
describe('add', () => {
  it('sends only the visible fields to the API', async () => {
    const {service, apiSpy} = createMockService();
    const view: View = {
      ...FW_LITE_VIEW,
      name: 'New View',
      // lexemeForm show:true, note show:true, others vary — just send shown ones
    };

    await service.add(view);

    expect(apiSpy.createCustomView).toHaveBeenCalledOnce();
    const sent: ICustomView = apiSpy.createCustomView.mock.calls[0][0];
    expect(sent.name).toBe('New View');
    // Only visible fields should be in the payload
    const sentIds = sent.entryFields.map(f => f.fieldId);
    expect(sentIds).toContain('lexemeForm');
    // complexFormTypes and publishIn are show:false in FW_LITE_VIEW — they must be omitted
    expect(sentIds).not.toContain('complexFormTypes');
    expect(sentIds).not.toContain('publishIn');
  });
...
describe('update', () => {
  it('sends the view id and visible fields to the API', async () => {
    const {service, apiSpy} = createMockService();
    const view: View = {...FW_LITE_VIEW, id: 'my-view', name: 'Updated View'};
