<script lang="ts">
  ...
  let allDomains = ['fruit', 'tree', 'stars', 'earth'];
  for (let i = 0; i < 100; i++) {
    allDomains.push(
      allDomains[Math.floor(Math.random() * allDomains.length)] + '-' + i
    );
  }
...
<div class="p-6 shadcn-root">
  ...
  <ResizablePaneGroup direction="horizontal">
    ...
    <ResizableHandle class="after:bg-border relative hidden w-3 bg-transparent p-0 after:absolute after:right-0 after:top-1/2 after:h-8 after:w-[6px] after:-translate-y-1/2 after:translate-x-[-1px] after:rounded-full after:transition-all after:hover:h-10 sm:block" />
    <ResizablePane>
    </ResizablePane>
  </ResizablePaneGroup>
  <div class="grid grid-cols-3 gap-6">
