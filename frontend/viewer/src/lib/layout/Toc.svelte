...
{#if entry}
  ...
  <div class="border flex flex-col rounded-md overflow-auto">
    <a class="toc-item px-2" href="#entry" title={_headword}><span class="opacity-80 mr-1">{fieldName({id: 'entry'}, $currentView.i18nKey)}:</span>{_headword || '—'}</a>
    {#each entry.senses as sense, i (sense.id)}
      {@const _sense = firstDefOrGlossVal(sense) ?? ''}
      <a class="toc-item px-2 pl-4" href="#sense{i + 1}" title={_sense}><span class="opacity-80 mr-1">{fieldName({id: 'sense'}, $currentView.i18nKey)}:</span>{_sense || '—'}</a>
