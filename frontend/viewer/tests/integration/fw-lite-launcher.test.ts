...
import type {LaunchConfig} from '../e2e/types';
import {getTestConfig} from '../e2e/config';

// Hard-fail if the binary is missing. Running this suite at all means you've opted in
// (via `pnpm test:integration` or the e2e CI job). Default `pnpm test` filters out the
// integration project, so unrelated workflows aren't burdened with building the binary.
const fwLiteBinaryPath = getTestConfig().fwLite.binaryPath;
