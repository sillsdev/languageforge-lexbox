...
//A TypeConverter is required so reflection-based consumers (SystemTextJsonPatch's
//DictionaryTypedPropertyProxy, ASP.NET Core route/config binding, ...) can convert
//string ↔ WritingSystemId. Without it, callers fall back to less-typed paths.
[JsonConverter(typeof(WritingSystemIdJsonConverter))]
[TypeConverter(typeof(WritingSystemIdTypeConverter))]
...
public class WritingSystemIdTypeConverter : TypeConverter
{
    public override bool CanConvertFrom(ITypeDescriptorContext? context, Type sourceType)
        => sourceType == typeof(string) || base.CanConvertFrom(context, sourceType);

    public override object? ConvertFrom(ITypeDescriptorContext? context, CultureInfo? culture, object value)
        => value is string s ? WritingSystemId.Parse(s, culture) : base.ConvertFrom(context, culture, value);

    public override bool CanConvertTo(ITypeDescriptorContext? context, Type? destinationType)
        => destinationType == typeof(string) || base.CanConvertTo(context, destinationType);

    public override object? ConvertTo(ITypeDescriptorContext? context, CultureInfo? culture, object? value, Type destinationType)
        => destinationType == typeof(string) && value is WritingSystemId wsId
            ? wsId.Code
