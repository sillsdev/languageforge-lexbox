...
public class CrdtRepairTests : IClassFixture<SyncFixture>, IAsyncLifetime
{
    ...
    public async Task SyncMissingTranslationId_IntoSnapshotAndCrdt()
    {
        ...
        crdtEntry.Senses.Single().ExampleSentences.Single().Translations.First().Id = Translation.DefaultFirstTranslationId;
        crdtEntry = await _fixture.CrdtApi.CreateEntry(crdtEntry);
        // make sure the constant Guid was actually persisted
        crdtEntry.Senses.Single().ExampleSentences.Single().Translations.First().Id.Should().Be(Translation.DefaultFirstTranslationId);
        ...
        updatedCrdtEntry.Senses.Single().ExampleSentences.Single().Translations.First().Id.Should().NotBe(Translation.DefaultFirstTranslationId);
    }
}
