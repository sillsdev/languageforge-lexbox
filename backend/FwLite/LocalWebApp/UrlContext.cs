...
public class UrlContext(IServer server, IHttpContextAccessor contextAccessor)
{
    ...
    public (Uri host, bool guess) GetUrl()
    {
        ...
        var address = server.Features.Get<IServerAddressesFeature>()?.Addresses.FirstOrDefault(a => a.StartsWith("http:")) ?? throw new InvalidOperationException("No server address");
