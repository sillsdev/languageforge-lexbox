...
When `IcuCollationRules` or `SystemCollationLocale` is set, use icu.net (`RuleBasedCollator` / locale `Collator`) `Compare` with no legacy case tie-break layered on top. Matches FLEx for custom and other-language modes. Android bundles ICU natives via `Icu4c.Android.Fw.Lib`.
